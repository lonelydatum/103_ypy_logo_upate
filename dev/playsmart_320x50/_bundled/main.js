...
function logoGO() {
  ...
  tl.to(["#bg", "#logo_olg"], { duration: 0.7, stagger: 0.1, scale: 1, ease: "back.out" });

  tl.to("#number", { duration: 0.3, opacity: 1 }, "-=.25");
  var PAUSE = 0.25;

  tl.set("#zero", { opacity: 1 });

  tl.add("heart", "+=" + PAUSE);
  tl.to("#zero", { duration: 0.1, opacity: 0 }, "heart");
  tl.to("#heart", { duration: 0.1, opacity: 1 }, "heart");

  tl.add("fire", "+=" + PAUSE);
  tl.to("#heart", { duration: 0.1, opacity: 0 }, "fire");
  tl.to("#fire", { duration: 0.5, scale: 1, ease: "back.out" }, "fire");

  tl.add("leaf", "+=" + PAUSE);
  tl.to("#fire", { duration: 0.1, scale: 0 }, "leaf");
  tl.to("#leaf", { duration: 0.1, opacity: 1 }, "leaf");

  tl.add("zero", "+=" + PAUSE);
  tl.to("#leaf", { duration: 0.1, opacity: 0 }, "zero");
  tl.to("#zero", { duration: 0.1, opacity: 1 }, "zero");
  return tl;
}
...
function olg() {
  ...
  var tl = new TimelineMax({
    onStart: function onStart() {
      if (document.querySelector(".olg-static")) {
        TweenLite.set(".olg-static", { opacity: 0 });
      }
    }
  });

  tl.to("#bluewedge1", { duration: 0.5, ease: "power1.inOut", scaleY: 1, scale: 1, x: 0, y: 0 }, 0);
  tl.to("#redwedge1", { duration: 0.8, ease: "power1.inOut", scaleY: 1, scale: 1, x: 0, y: 0 }, 0).from("#group-o", { duration: 1, y: 200, ease: "custom" }, 0).from("#group-l", { duration: 1, y: 200, ease: "custom" }, 0.1).from("#group-g", { duration: 1, y: 200, ease: "custom" }, 0.2).from("#group-o", { duration: 0.8, scale: 0.4, ease: "power1.out" }, 0.3).from("#group-l", { duration: 0.8, scale: 0.4, ease: "power1.out" }, 0.4).from("#group-g", { duration: 0.8, scale: 0.4, ease: "power1.out" }, 0.5).from("#letter-o", { duration: 0.25, scale: 0, ease: "back.out(2)", svgOrigin: "28pt 75pt" }, 0.9).from("#letter-l", { duration: 0.25, scale: 0, ease: "back.out(2)", svgOrigin: "55pt 75pt" }, 1).from("#letter-g", { duration: 0.25, scale: 0, ease: "back.out(2)", svgOrigin: "80pt 75pt" }, 1.1);
...
function init(_ref) {
  ...
  tl.add("end", "+=.3");
  tl.from(".end_url", { duration: 0.3, opacity: 0 }, "end");
  tl.add((0, _commonJsCommonJs.ypyScroll)(), "end");

  tl.from(".end_device", { duration: 0.3, opacity: 0 }, "end");
  tl.from(".end_cta", { duration: 0.3, opacity: 0, y: "+=50", opacity: 0 }, "+=.3");
  tl.to("._logo", { duration: 0.3, y: "+=90" });
